...
const HotelList = () => {
    ...
    const [location, setLocation] = useState(''); // State for location search

    useEffect(() => {
        const find_avail_hotels = async () => {
            try {
                const { data } = await get_avail_hotels({ location }); // Pass location to the API
                setHotels(data);
            } catch (err) {
        ...
        find_avail_hotels();
    }, [location]); // Reload hotels when location changes
